...
import pytest
from pykeyboard import InlineKeyboard, InlineButton, PaginationError, LocaleError
...
class TestInlineKeyboard:
    ...
    def test_pagination_edge_cases(self):
        ...
        keyboard = InlineKeyboard()
        with pytest.raises(PaginationError, match="current_page.*cannot exceed count_pages"):
    ...
    def test_language_selection_invalid_callback_pattern(self):
        ...
        with pytest.raises(LocaleError, match="callback_pattern must contain '\\{locale\\}' placeholder"):
    ...
    def test_language_selection_mixed_valid_invalid_locales(self):
        ...
        keyboard = InlineKeyboard()
        keyboard.languages("lang_{locale}", ["en_US", "invalid", "ru_RU", "also_invalid"], 2)
    ...
    def test_language_selection_large_number_of_locales(self):
        ...
        locales = ["en_US", "ru_RU", "de_DE", "fr_FR", "es_ES", "it_IT", "pt_BR", "zh_CN", "ja_JP", "ko_KR"]
        keyboard = InlineKeyboard()
        keyboard.languages("lang_{locale}", locales, 3)

        # Should create multiple rows
        assert len(keyboard.keyboard) == 4  # 10 locales with 3 per row = 4 rows (3 + 3 + 3 + 1)
        assert len(keyboard.keyboard[0]) == 3
    ...
    def test_language_selection_duplicate_locales(self):
        ...
        keyboard = InlineKeyboard()
        keyboard.languages("lang_{locale}", ["en_US", "ru_RU", "en_US", "ru_RU"], 2)
    ...
    def test_language_selection_non_string_non_list_locales(self):
        ...
        with pytest.raises(LocaleError, match="locales must be a list or string"):
    ...
    def test_json_serialization(self):
        """Test keyboard JSON serialization and deserialization."""
        keyboard = InlineKeyboard()
        keyboard.paginate(5, 3, "page_{number}")
        keyboard.add(InlineButton("Extra", "extra"))

        # Serialize to JSON
        json_str = keyboard.to_json()
        assert isinstance(json_str, str)
        assert '"count_pages":5' in json_str
        assert '"current_page":3' in json_str
    ...
    def test_json_serialization_complex_keyboard(self):
        """Test JSON serialization with complex keyboard structures."""
        keyboard = InlineKeyboard()
        ...
        assert len(restored.keyboard) == len(keyboard.keyboard)
        assert len(restored.keyboard[0]) == len(keyboard.keyboard[0])
    ...
    def test_pagination_invalid_callback_pattern(self):
        ...
        with pytest.raises(PaginationError, match="callback_pattern must contain '\\{number\\}' placeholder"):
    ...
    def test_keyboard_large_button_count(self):
        ...
        # Add 50 buttons
        buttons = [InlineButton(f"Button {i}", f"callback_{i}") for i in range(50)]
    ...
    def test_keyboard_mixed_button_types(self):
        ...
        button3 = InlineButton("Complex Button",
    ...
    def test_keyboard_chaining_operations(self):
        ...
        assert len(keyboard.keyboard) == 4  # 2 rows from add, 1 from first row, 1 from second row
    ...
    def test_custom_locale_validation(self):
        ...
        # Invalid empty code
        with pytest.raises(ValueError, match="locale_code must be a non-empty string"):
    ...
    def test_custom_locale_override_builtin(self):
        ...
        all_locales = keyboard.get_all_locales()
        assert all_locales["en_US"] == "Custom English"  # Should be custom, not built-in