"""Type-safe keyboard builder and factory methods."""

from typing import List, Union, Optional, Any, Callable, Dict, TypeVar
from .keyboard_base import KeyboardBase, InlineButton, Button
from .inline_keyboard import InlineKeyboard
from .reply_keyboard import ReplyKeyboard, ReplyButton
from functools import wraps
from .errors import ValidationError, PaginationError, LocaleError, ConfigurationError
...
class KeyboardBuilder:
    ...
    def add_validation_hook(self, hook: Callable[[Any], bool]) -> 'KeyboardBuilder':
    ...
    def add_button_transform(self, transform: Callable[[Any], Any]) -> 'KeyboardBuilder':
    ...
    def _create_button_from_spec(self, btn_spec: Union[str, Dict[str, Any], Any]) -> Any:
    ...
    def add_button(self, text: str, callback_data: Optional[str] = None, **kwargs) -> 'KeyboardBuilder':
        ...
        """
        if isinstance(self.keyboard, InlineKeyboard):
            button = InlineButton(text=text, callback_data=callback_data, **kwargs)
        else:
    ...
    def add_buttons(self, *buttons: Union[str, Dict[str, Any], Any]) -> 'KeyboardBuilder':
    ...
    def add_row(self, *buttons: Union[str, Dict[str, Any], Any]) -> 'KeyboardBuilder':
        ...
        try:
            ...
            return self
        except (PaginationError, ValidationError, LocaleError, ConfigurationError) as e:
            raise e
    ...
    def add_paginated_buttons(
        ...
        for item in page_items:
            callback_data = callback_pattern.format(item=item, page=current_page)
...
class KeyboardFactory:
    ...
    def create_confirmation_keyboard(
        ...
        builder.add_button(yes_text, callback_pattern.format(action="yes"))
        builder.add_button(no_text, callback_pattern.format(action="no"))

        if cancel_text:
            # Ensure cancel button has proper callback_data; pass as dict spec to avoid creating a text-only button
            builder.add_row({"text": cancel_text, "callback_data": callback_pattern.format(action="cancel")})
    ...
    def create_menu_keyboard(
        ...
        for text, action in menu_items.items():
            buttons.append({"text": text, "callback_data": callback_pattern.format(action=action)})
    ...
    def create_rating_keyboard(
        ...
        for i in range(1, max_rating + 1):
            ...
            builder.add_button(text, callback_pattern.format(stars=i))
    ...
    def create_pagination_keyboard(
        ...
        builder.add_navigation_buttons(total_pages, current_page, callback_pattern)
        ...
        if include_buttons:
            ...
            for btn in include_buttons:
                button_specs.append({
                    "text": btn["text"],
                    "callback_data": btn.get("callback_data", btn["text"].lower())
                })
...
def keyboard_factory(func: Callable[..., Union[InlineKeyboard, ReplyKeyboard]]) -> Callable[..., Union[InlineKeyboard, ReplyKeyboard]]:
